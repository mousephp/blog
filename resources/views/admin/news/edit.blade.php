...
@section('title','Sửa News')

@section('content')
<div class="container">
	<div class="col-sm-12">
		<h2 class="">SỬA TIN TỨC</h2>
		<hr>
		<form method="post" action="{{route('news.update',$new->id)}}" class="form-horizontal" enctype="multipart/form-data">
			@csrf
			@method('PUT')
			<div class="form-group ">
				<label for="example-text-input" class="col-sm-2 col-form-label">Tiêu đề</label>
				<div class="col-sm-8">
					<input class="form-control" type="text" value="{{$new->news_title}}" id="example-text-input" name="title" placeholder="NHập Tên tác giả">
					@if ($errors->has('title'))
			...
			<div class="form-group ">
				...
				<div class="col-sm-8">
					<select class="form-control input-width" name="author">
						...
						<option @if($new->author_id==$value->id)
							{{'selected'}} @endif
							value="{{$value->id}}">{{$value->author_name}}</option>
						}
						@endforeach
					</select>
				</div>
			...
			<div class="form-group ">
				...
				<div class="col-sm-8">
					<textarea id="category-summary"  class="form-control" name="summary" rows="7" cols="50">{{$new->news_summary}}</textarea>
			...
			<div class="form-group ">
				...
				<div class="col-sm-8">
					<textarea id="category-desciption"  class="form-control" name="desciption" rows="7" cols="50">{{$new->news_content}}</textarea>
					<script type="text/javascript">
			...
			<div class="form-group ">
				...
				<div class="col-sm-8">
					<select class="form-control input-width" name="category">
						...
						<option @if($new->category_id==$value->id)
							{{'selected'}} @endif
				...
				<div class="col-sm-8">
					<select class="form-control input-width" name="tags">					
						...
						<option @if($new->tag_id==$value->id)
							{{'selected'}} @endif
							value="{{$value->id}}">{{$value->tag_title}}</option>
						}
						@endforeach
					</select>
				</div>
